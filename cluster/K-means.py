...
kmeans.fit(dataset)  # 使用评估的带宽构建均值漂移模型，并进行训练
labels = kmeans.labels_
cluster_num = len(np.unique(labels))  # 生成聚类标签
centroids = kmeans.cluster_centers_  # 返回蔟的中心点，这些点是不一定是样本点，该点的坐标对应于各坐标轴
...
visual_cluster_effect(kmeans, dataset, 'MeanShift-X=fresh,y=milk', [0, 1])  # X=fresh， y=milk
# 使用轮廓系数评估模型的优劣
from sklearn.metrics import silhouette_score

si_score = silhouette_score(dataset, kmeans.labels_,
